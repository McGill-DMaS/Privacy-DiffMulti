...
bool CTDController::runDiffMulti()
{
	...
	// Write the .names file for Discernibility and NCP 
	...
	// Write the .names file for classification or clustering
	if (!m_attribMgr.writeNameFileMultiDim())
		return false;	
	
	// Print records in binary format
	if (!m_dataMgr.writeMultiDimRecords(m_partitioner.getLeafPartitions(), m_partitioner.getTestLeafPartitions(), TD_bC45, TD_bCluster))	
		return false;
#endif
