...
// Where:
//  <class_label> is the actual class label for C4.5, or +1 or -1 for SVM.
//  <concept#>    is an integer >= 1 representing the index of a concept value within the set 
//                of all concept values in all leaf partitions. Should always be increasing.
//  <value>
//  For categorical attributes:
//                <value> = 0 or 1. Every categorical attribute will be replaced a set of 
//				  concepts to which records have been generalized.
//  For numerical attributes:
//   For C4.5:    <value> is the midpoint of the interval to which the 
//                raw numerical value has been generalized.
//   For SVM:     <value> is the normalized raw value within the full numerical domain ([0-1]). 
//                However, raw values are being generalized to intervals in our approach. 
//                Hence, the generalization interval is normalized with respect to the full domain range.
//            
// Clustering (vcluster.exe is from CLUTO, Karypis Lab):
// First line:    <#records> <#attributes>
// Rest of lines: <value>,...,<value>
// Where:
//  <#attributes> is the number of generalizatin concepts to which records have been generalized.
//				  An attribute will be replaced by such set of concepts.
//	<value>
//  For categorical attributes:
//                <value> = 0 or 1. Every categorical attribute will be replaced a set of 
//				  concepts to which records have been generalized.
//  For numerical attributes:
//				  <value> is normalized over its interval.
//---------------------------------------------------------------------------
bool CTDDataMgr::writeMultiDimRecords(CTDPartitions* pLeafPartitions, CTDPartitions* pTestLeafPartitions, bool isC45, bool isCluster)
{
	cout << _T("Writing multidimensional records...") << endl;
	int longestPath = 0;
    ...
    try {
		...
		// Write records for clustering
		if (isCluster) {
			if (!writeVectorRecordsForVCluster(transDataFile)) {
				cerr << _T("Failed to write records for clustering") << endl;
				ASSERT(false);
				return false;
			}
			...
			// Write records for classification
...
//---------------------------------------------------------------------------
// Prepare records for vcluster.exe (from CLUTO, Karypis Lab)
// Output records must be in the same order as input records
//---------------------------------------------------------------------------
bool CTDDataMgr::writeVectorRecordsForVCluster(CStdioFile& vectorFile)
{
	// Write the number of records and attributes
    LPCTSTR tempStr1 = CTDContConcept::FloatToStr(m_records.GetSize());
    LPCTSTR tempStr2 = CTDContConcept::FloatToStr(countNumVectorAttribs(false));
    vectorFile.WriteString(CString(tempStr1) + _T(" ") + CString(tempStr2) + _T("\n"));
    delete [] tempStr1;
    delete [] tempStr2;
    tempStr1 = NULL;
    tempStr2 = NULL;
	...
	int nRecords = m_records.GetSize();
    for (int i = 0; i < nRecords; ++i) {
        vectorFile.WriteString(m_records.GetAt(i)->toVectorStringForVCluster(false, _T(" ")) + _T("\n"));
    }
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
int CTDDataMgr::countNumVectorAttribs(bool bIncludeClassAttrib)
{
    int nVectorAttribs = 0;
    CTDAttrib* pAttrib = NULL;
    CTDAttribs* pAttribs = m_pAttribMgr->getAttributes();
    ...
    for (int a = 0; a < nAttribs; ++a) {
        pAttrib = pAttribs->GetAt(a);
        if (pAttrib->isContinuous())
            ++nVectorAttribs;
        else
			nVectorAttribs += pAttrib->getMultiDimConcepts()->GetSize();
    }
    return nVectorAttribs;
}