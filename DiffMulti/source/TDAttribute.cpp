...
CTDAttrib::CTDAttrib(LPCTSTR attribName)
	...
	m_attribIdx(-1), 
	m_nOFlatConcepts(-1), 
	m_bVirtualAttrib(false), 
	m_bMaskTypeSup(false)
{
}
...
//---------------------------------------------------------------------------
// Tree: {Any Location {BC {Vancouver} {Surrey} {Richmond}} {AB {Calgary} {Edmonton}}}
// m_flattenConcepts will group concepts of the same level together:
// {Any Location, BC, AB, Vancouver, Surrey, Richmond, Calgary, Edmonton}
//---------------------------------------------------------------------------
...
bool CTDDiscAttrib::initHierarchy(LPCTSTR conceptStr)
{
    ...
    if (!m_pConceptRoot)
        return false;
	if (!m_pConceptRoot->initHierarchy(conceptStr, 0, m_reqBits, m_maxDepth))
        return false;
    if (m_bMaskTypeSup) {
...
CTDContAttrib::CTDContAttrib(LPCTSTR attribName) 
    : CTDAttrib(attribName)
{
	m_maxDepth = TD_CONT_ATTR_LEVELS;
}
